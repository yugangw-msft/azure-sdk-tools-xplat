  ...
  describe('network', function () {
    ...
    describe('network watchers', function () {
      ...
      it('check-connectivity should perform check connectivity operation successfully', function (done) {
        var cmd = 'network watcher check-connectivity -g {group} -n {name} --source-id {virtualMachineId} --destination-address {address} --destination-port {destinationPort} --json'.formatArgs(networkWatchers);
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
