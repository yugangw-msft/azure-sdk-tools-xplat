  ...
  describe('network', function () {
    ...
    describe('network watchers', function () {
      ...
      it('topology should perform get topology operation successfully', function (done) {
        var cmd = 'network watcher topology -g {group} -n {name} --topology-resource-group {topology_targetResourceGroupName} --json'.formatArgs(networkWatchers);
        generatorUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('next-hop should perform get next hop operation successfully', function (done) {
        var cmd = 'network watcher next-hop -g {group} -n {name} --target {virtualMachineId} --source-ip-address {nextHop_sourceIPAddress} --destination-ip-address {nextHop_destinationIPAddress} --nic-id {networkInterfaceId} --json'.formatArgs(networkWatchers);
        generatorUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('configure-flow-log should perform set flow log configuration operation successfully', function (done) {
        var cmd = 'network watcher configure-flow-log -g {group} -n {name} --target {networkSecurityGroupId} --enable {configureFlowLog_enabled} --storage-id {storageId} --retention-enable {configureFlowLog_retentionPolicyEnabled} --retention-days {configureFlowLog_days} --json'.formatArgs(networkWatchers);
        generatorUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('check-connectivity should perform check connectivity operation successfully', function (done) {
        var cmd = 'network watcher check-connectivity -g {group} -n {name} --source-id {virtualMachineId} --destination-address {checkConnectivity_address} --destination-port {checkConnectivity_destinationPort} --json'.formatArgs(networkWatchers);
        generatorUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('reachability-report-provider-list should perform list available providers operation successfully', function (done) {
        var cmd = 'network watcher reachability-report-provider-list -g {group} -n {name} --azure-locations {reachabilityReportProviderList_azureLocations} --country {reachabilityReportProviderList_country} --state {reachabilityReportProviderList_state} --city {reachabilityReportProviderList_city} --json'.formatArgs(networkWatchers);
        generatorUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
