      ...
      it('snapshot config set, snapshot create should pass', function(done) {
        ...
        vmTest.createGroup(groupName, location, suite, function(result) {
          ...
          testUtils.executeCommand(suite, retry, cmd, function(result) {
            ...
            testUtils.executeCommand(suite, retry, cmd, function(result) {
              result.exitStatus.should.equal(0);
              var cmd = makeCommandStr('snapshot', 'set', paramFileName, util.format('--account-type %s', 'Standard_LRS')).split(' ');
              testUtils.executeCommand(suite, retry, cmd, function(result) {
                result.exitStatus.should.equal(0);
      ...
      it('snapshot update parameters, snapshot delete should pass', function(done) {
        ...
        testUtils.executeCommand(suite, retry, cmd, function(result) {
          ...
          var cmd = makeCommandStr('snapshot', 'set', updateFileName, util.format('--name %s --location %s', diskPrefix, location)).split(' ');
            testUtils.executeCommand(suite, retry, cmd, function(result) {
              result.exitStatus.should.equal(0);
              var cmd = makeUpdateParametersCommandStr('snapshot-update', 'set', updateFileName, util.format('--account-type %s --os-type %s --encryption-settings %s --tags %s', 'Premium_LRS', 'Windows', 'null', 'testtag')).split(' ');
              testUtils.executeCommand(suite, retry, cmd, function(result) {
                result.exitStatus.should.equal(0);
