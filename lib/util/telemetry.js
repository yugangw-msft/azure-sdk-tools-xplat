...
exports.currentCommand = function (command) {
  if (command && typeof command === 'object') {
    ...
    if (_event) {
      ...
      // By now we have all information about current command,
      // set useragent data to be sent as part of request header.
...
function constructUserAgentData() {
  if (_event) {
    return {
      ...
      mode: _event.mode,
      commandName: _event.commandName,
      rawCommand: _rawCommand.slice(2, _rawCommand.length).join(' ')
    };
  }
}
