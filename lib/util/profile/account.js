...
Account.prototype._getSubscriptionInCloudConsole = function (callback) {
  ...
  var tokenEntry = self._convertToken(tokens[0], self._env.managementEndpointUrl);
  //TODO: verify ADAL has the same query
  self._tokenCache.find({
    '_clientId': tokenEntry._clientId,
    '_authority': tokenEntry._authority,
    'userId': tokenEntry.userId,
    'resource': tokenEntry.resource
  }, function (err, entries) {
    //no err check, as find will not return any
    self._tokenCache.remove(entries, function (err) {
      if (err) {
        return callback(err);
      }
      self._tokenCache.add([tokenEntry], function (err) {
        if (err) {
          return callback(err);
        }
        var tenant = tokenEntry._authority.split('/').pop();
        self._username = tokenEntry.userId;
        var credential = new self._adalAuth.UserTokenCredentials(self._env.getAuthConfig(tenant), tokenEntry.userId);
        var armClient = self._getArmClient(credential);
        var subscriptions = [];
        armClient.subscriptions.list(function (err, result) {
          if (!err) {
...
Account.prototype._convertToken = function (accessToken, resource) {
  decodedToken = jwtDecode(accessToken);
  return {
    '_clientId': constants.XPLAT_CLI_CLIENT_ID,
    'expiresIn': '3600', //not accurate, but okay, as portal invokes us very 15 mins
    'expiresOn': (new Date()).add({ seconds : 3600 }),
    'userId': decodedToken.upn,
