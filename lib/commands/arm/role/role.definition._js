...
exports.init = function (cli) {
  ...
  role.command('set [inputfile] [roledefinition]')
    ...
    try {
      var getResult;
      for(var i=0; i < parameters.roleDefinition.properties.assignableScopes.length; i++) {
        var scope = parameters.roleDefinition.properties.assignableScopes[i];
        roleUtils.validateScope(scope);
        getResult = authzClient.roleDefinitions.get(roleDefinitionIdGuid, scope, _).roleDefinition;
        if(getResult){
          break;
        }
      }
