...
exports.init = function (cli) {
  ...
  dataLakeStoreFileSystem.command('read [accountName] [path] [length] [offset]')
    ...
    if (parameters.offset >= fileInfo.length) {
      throw new Error($('--offset must be less than than the length of the file. File length: ' + fileInfo.length + '. Value passed in: ' + offset));
    }
    ...
    parameters.length = parseInt(length);
  ...
  dataLakeStoreFirewallRules.command('create [accountName] [resource-group] [name] [startIpAddress] [endIpAddress]')
    .description($('adds the specified firewall rule to the specified Data Lake Store account.'))
    .usage('[options] <accountName> <resource-group> <name> <startIpAddress> <endIpAddress>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    ...
    .option('-f --name <firewall rule name>', $('the name of the firewall rule to add'))
    ...
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, name, startIpAddress, endIpAddress, options, _) {
    if (!accountName) {
      return cli.missingArgument('accountName');
    }
    ...
    var result = client.firewallRules.createOrUpdate(resourceGroup, accountName, name, parameters, _);
    dataLakeStoreUtils.formatOutput(cli, log, options, result);
  });
  
  dataLakeStoreFirewallRules.command('set [accountName] [resource-group] [name] [startIpAddress] [endIpAddress]')
    .description($('updates the specified firewall rule in the specified Data Lake Store account.'))
    .usage('[options] <accountName> <resource-group> <name> <startIpAddress> <endIpAddress>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    .option('-g --resource-group <resource-group>', $('optionally explicitly set the resource group. If not specified, will attempt to discover it.'))
    .option('-f --name <firewall rule name>', $('the name of the firewall rule to update'))
    ...
    var client = utils.createDataLakeStoreManagementClient(subscription);
    var currentRule = client.firewallRules.get(resourceGroup, accountName, name, _);
    if (!startIpAddress) {
      startIpAddress = currentRule.startIpAddress;
    }
    
    if (!endIpAddress) {
      endIpAddress = currentRule.endIpAddress;
    }
  ...
  dataLakeStoreFirewallRules.command('show [accountName] [resource-group] [name]')
    .description($('retrieves the specified firewall rule in the specified Data Lake Store account.'))
    .usage('[options] <accountName> <resource-group> <name>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    .option('-g --resource-group <resource-group>', $('optionally explicitly set the resource group. If not specified, will attempt to discover it.'))
    .option('-f --name <firewall rule name>', $('the name of the firewall rule to display'))
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, name, options, _) {
    if (!accountName) {
      return cli.missingArgument('accountName');
    }
  ...
  dataLakeStoreFirewallRules.command('list [accountName] [resource-group]')
    ...
    var client = utils.createDataLakeStoreManagementClient(subscription);
    var response = client.firewallRules.listByAccount(resourceGroup, accountName, _);
    var rules = response;
  ...
  dataLakeStoreFirewallRules.command('delete [accountName] [resource-group] [name]')
    ...
    .option('-f --name <firewall rule name>', $('the name of the firewall rule to delete'))
    .option('-q, --quiet', $('quiet mode (do not ask for delete confirmation)'))
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, name, options, _) {
    if (!accountName) {
    ...
    var client = utils.createDataLakeStoreManagementClient(subscription);
    client.firewallRules.deleteMethod(resourceGroup, accountName, name, _);
  ...
  dataLakeStoreTrustedIdProvider.command('create [accountName] [resource-group] [name] [providerEndpoint]')
    .description($('adds the specified trusted identity provider to the specified Data Lake Store account.'))
    .usage('[options] <accountName> <resource-group> <name> <providerEndpoint>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    .option('-g --resource-group <resource-group>', $('optionally explicitly set the resource group. If not specified, will attempt to discover it.'))
    .option('-t --name <trusted identity provider name>', $('the name of the trusted identity provider to add'))
    ...
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, name, providerEndpoint, options, _) {
    createOrUpdateTrustedIdProvider(cli, accountName, resourceGroup, name, providerEndpoint, options, _);
  });
  
  dataLakeStoreTrustedIdProvider.command('set [accountName] [resource-group] [name] [providerEndpoint]')
    .description($('updates the specified trusted identity provider in the specified Data Lake Store account.'))
    .usage('[options] <accountName> <resource-group> <name> <providerEndpoint>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    .option('-g --resource-group <resource-group>', $('optionally explicitly set the resource group. If not specified, will attempt to discover it.'))
    .option('-t --name <trusted identity provider name>', $('the name of the trusted identity provider to update'))
    .option('-e, --providerEndpoint <endpoint>', $('the new valid trusted provider endpoint in the format: https://sts.windows.net/<provider identity>'))
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, name, providerEndpoint, options, _) {
    createOrUpdateTrustedIdProvider(cli, accountName, resourceGroup, name, providerEndpoint, options, _);
  ...
  dataLakeStoreTrustedIdProvider.command('show [accountName] [resource-group] [name]')
    .description($('retrieves the specified trusted identity provider in the specified Data Lake Store account.'))
    .usage('[options] <accountName> <resource-group> <name>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    .option('-g --resource-group <resource-group>', $('optionally explicitly set the resource group. If not specified, will attempt to discover it.'))
    .option('-t --name <trusted identity provider name>', $('the name of the trusted identity provider to display'))
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, name, options, _) {
    if (!accountName) {
      return cli.missingArgument('accountName');
    ...
    var client = utils.createDataLakeStoreManagementClient(subscription);
    var currentRule = client.trustedIdProviders.get(resourceGroup, accountName, name, _);
    dataLakeStoreUtils.formatOutput(cli, log, options, currentRule);
  });
  
  dataLakeStoreTrustedIdProvider.command('list [accountName] [resource-group]')
    ...
    var client = utils.createDataLakeStoreManagementClient(subscription);
    var response = client.trustedIdProviders.listByAccount(resourceGroup, accountName, _);
    var rules = response;
    while (response.nextLink) {
  ...
  dataLakeStoreTrustedIdProvider.command('delete [accountName] [resource-group] [name]')
    .description($('removes the specified trusted identity provider in the specified Data Lake Store account.'))
    .usage('[options] <accountName> <resource-group> <name>')
    .option('-n --accountName <accountName>', $('the Data Lake Store account name to execute the action on'))
    .option('-g --resource-group <resource-group>', $('the optional resource group to list the accounts in'))
    .option('-f --name <trusted identity provider name>', $('the name of the trusted identity provider to delete'))
    .option('-q, --quiet', $('quiet mode (do not ask for delete confirmation)'))
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, name, options, _) {
    if (!accountName) {
    ...
    var client = utils.createDataLakeStoreManagementClient(subscription);
    client.trustedIdProviders.deleteMethod(resourceGroup, accountName, name, _);
  ...
  dataLakeStoreAccount.command('create [accountName] [location] [resource-group] [defaultGroup] [encryption] [keyVaultId] [keyName] [keyVersion]')
    ...
    .option('-s --keyVersion <key vault id>', $('if the encryption type is UserAssigned, this is the key version of the key the user wishes to use'))
    ...
    var dataLakeStoreAccount = createOrUpdateDataLakeStoreAccount(subscription, accountName, resourceGroup, location, defaultGroup, tags, config, _);
    dataLakeStoreUtils.formatOutput(cli, log, options, dataLakeStoreAccount);
  });
  
  dataLakeStoreAccount.command('set [accountName] [resource-group] [defaultGroup] [encryption] [keyVaultId] [keyName] [keyVersion]')
    ...
    .option('-e --encryption <\'UserManaged\'|\'ServiceManaged\'>', $('optionally indicates what type of encryption to provision the account with, if any. Valid values are UserManaged or ServiceManaged'))
    .option('-k --keyVaultId <key vault id>', $('if the encryption type is UserAssigned, this is the id of the key vault the user wishes to use'))
    .option('-y --keyName <key vault id>', $('if the encryption type is UserAssigned, this is the key name in the key vault the user wishes to use'))
    .option('-s --keyVersion <key vault id>', $('if the encryption type is UserAssigned, this is the key version of the key the user wishes to use'))
    .option('-t --tags <tags>', $('Tags to set to the Data Lake Store account. Can be mutliple. ' +
            'In the format of \'name=value\'. Name is required and value is optional. For example, -t tag1=value1;tag2'))
    .option('--no-tags', $('remove all existing tags'))
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, resourceGroup, defaultGroup, encryption, keyVaultId, keyName, keyVersion, options, _) {
    var subscription = profile.current.getSubscription(options.subscription);
    var client = utils.createDataLakeStoreManagementClient(subscription);
    ...
    if (!defaultGroup) {
      defaultGroup = dataLakeStoreAccount.defaultGroup;
    }
    ...
    dataLakeStoreAccount = createOrUpdateDataLakeStoreAccount(subscription, accountName, resourceGroup, dataLakeStoreAccount.location, defaultGroup, tags, config, _);
    dataLakeStoreUtils.formatOutput(cli, log, options, dataLakeStoreAccount);
  });
  
  function createOrUpdateDataLakeStoreAccount(subscription, accountName, resourceGroup, location, defaultGroup, tags, config, _) {
    if (!accountName) {
      return cli.missingArgument('accountName');
    }
    if (!location) {
    ...
    if (config) {
      ...
      accountParams.encryptionConfig = config;
