  ...
  try {
    for(var i=0 ;i<objectIds.length; i+=1000){
        ...
        if((i+1000) > objectIds.Count){
          resultObject=graphClient.objects.getObjectsByObjectIds({ objectIds: objectIds.slice(i, objectIds.length), includeDirectoryObjectReferences: true }, _);
        }
        else{
          resultObject=graphClient.objects.getObjectsByObjectIds({ objectIds: objectIds.slice(i,i+1000), includeDirectoryObjectReferences: true }, _);
        }

        if(resultObject && resultObject.length>0)
        {
          objects.push(resultObject[0]);
        }
    }
  } catch (ex) {
