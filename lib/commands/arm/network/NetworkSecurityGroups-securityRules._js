...
exports.init = function (cli) {
  ...
  securityRules.command('create [resource-group] [nsg-name] [name] [priority]')
    ...
    .execute(function(resourceGroup, nsgName, name, priority, options, _) {
      ...
      try {
        result = networkManagementClient.securityRules.get(resourceGroup, nsgName, name, null, _);
      } catch (e) {
        if (e.statusCode === 404) {
          result = null;
        } else {
      ...
      if(options.destinationPortRange) {
        ...
        parameters.destinationPortRange = '80';
      }
      ...
      try {
        result = networkManagementClient.securityRules.createOrUpdate(resourceGroup, nsgName, name, parameters, _);
      } finally {
        progress.end();
      }
  ...
  securityRules.command('delete [resource-group] [nsg-name] [name]')
    ...
    .execute(function(resourceGroup, nsgName, name, options, _) {
      ...
      try {
        result = networkManagementClient.securityRules.deleteMethod(resourceGroup, nsgName, name, _);
    ...
    .execute(function(resourceGroup, nsgName, options, _) {
      ...
      try {
        if(typeof networkManagementClient.securityRules.listAll != 'function') {
          ...
          result = networkManagementClient.securityRules.list(resourceGroup, nsgName,  _);
        } else {
          if(options.resourceGroup) {
            ...
            nsgName = cli.interaction.promptIfNotGiven($('network security group : '), nsgName, _);
            progress = cli.interaction.progress($('Getting the security rules'));
            result = networkManagementClient.securityRules.listAll(nsgName, _);
          }
        }
      } finally {
        progress.end();
