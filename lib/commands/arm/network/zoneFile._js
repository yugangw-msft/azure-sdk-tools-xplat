...
__.extend(ZoneFile.prototype, {
  ...
  checkCaaMultilineIssues: function(text) {
    ...
    for(i = 0; i < splitText.length; i++) {
      var current = splitText[i];
      if((current.match(caaRegex) && !current.match(braceRegex)) ||
         (current.match(braceRegex)) && i > 0 && splitText[i - 1].match(caaRegex)) {
        throw new Error(util.format($('Import file contains multiline record for CAA in:\r\n            %s\r\nPlease change this to single line try again.'), current));
      }
    }
  },
  ...
  parseCAA: function (rr) {
    ...
    var caaArray = recordSetUtils.splitTxtRecord(value);
    if(caaArray === '') {
      self.output.warn(util.format('Parsing of CAA value failed. Record would be ignored. Record set: %s; Input data: flags=%s, tag=%s, value=%s',
        rr.name, rr.data[0], rr.data[1], rr.data.slice(2)));
      return;
    }

    var caaValue = caaArray[0];
    var caa = {
      name: rr.name,
      type: rr.type,
      ttl: rr.ttl,
