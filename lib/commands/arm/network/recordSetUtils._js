...
exports.covertToAzureFormat = function (recordSet) {
  ...
  switch (recordSet.type) {
    ...
    case constants.dnsZone.CAA :
      parameters.caaRecords = recordSet.records;
      break;
...
exports.convertToListFormat = function (recordSets) {
  ...
  for (var i = 0; i < recordSets.length; i++) {
    ...
    if (!__.isEmpty(recordSet.caaRecords)) {
      ...
      __.each(recordSet.caaRecords, function (rec) {
        item.records.push(util.format('%s %s %s', rec.flags, rec.tag, rec.value));
      });
      items.push(item);
    }
...
exports.removeEmptyRecords = function (recordSet) {
  ...
  var fields = ['aRecords', 'aaaaRecords', 'nsRecords', 'MXRecords', 'srvRecords', 'txtRecords', 'soaRecord', 'ptrRecords', 'caaRecords'];
...
function mergeCAA(rs1, rs2) {
  var caaRecords = rs1.caaRecords;
  var caaRecordsExisting = __.clone(rs2.caaRecords);
  for (var i = 0; i < caaRecords.length; i++) {
    for (var j = 0; j < caaRecordsExisting.length; j++) {
      if (caaRecords[i].flags === caaRecordsExisting[j].flags &&
          caaRecords[i].tag.toLowerCase() === caaRecordsExisting[j].tag.toLowerCase() &&
          caaRecords[i].value === caaRecordsExisting[j].value) {
        rs2.caaRecords.push(caaRecords[i]);
      }
    }
  }
}
