...
function mergeCAA(rs1, rs2) {
  ...
  for (var i = 0; i < caaRecords.length; i++) {
    var result = rs2.caaRecords.some(function(element) {
      return (caaRecords[i].flags === element.flags &&
        caaRecords[i].tag.toLowerCase() === element.tag.toLowerCase() &&
        caaRecords[i].value === element.value);
    });
    if(!result) {
