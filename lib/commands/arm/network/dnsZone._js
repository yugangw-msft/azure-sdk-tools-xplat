  ...
  tryImportRecordSet: function (resourceGroupName, zoneName, setName, setType, parameters, _) {
    ...
    try {
      if (setName === '@' && (setType === constants.dnsZone.SOA || setType === constants.dnsZone.NS)) {
        ...
        var result = self.getRecordSet(resourceGroupName, zoneName, setName, { type: setType }, _);
        if(result) {
          parameters = recordSetUtils.merge(parameters, result, setType, {}, self.output);
        }
    ...
    if (options.caaflags || options.caatag || options.caavalue) {
      if (options.type !== constants.dnsZone.CAA) {
        ...
        if (isAdding) {
          ...
          if (recordIndex === -1) {
            throw new Error(util.format($('Record of type "%s" with flags="%s",tag="%s",value="%s" not found in the record set "%s"'),
              options.type, options.flags, options.tag, options.value, recordSet.name));
          }
