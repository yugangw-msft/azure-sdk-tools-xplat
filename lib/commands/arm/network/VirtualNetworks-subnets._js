...
exports.init = function (cli) {
  ...
  subnets.command('create [resource-group] [vnet-name] [name] [address-prefix]')
    ...
    .option('-p, --private-access-services [private-access-services]', $('an array of private access services' +
      '\n     values'))
    ...
    .execute(function(resourceGroup, vnetName, name, addressPrefix, options, _) {
      ...
      if(options.privateAccessServices) {
        parameters.privateAccessServices = options.privateAccessServices.split(',').map(function(item) { return { service: item }; });
      }
  ...
  subnets.command('set [resource-group] [vnet-name] [name]')
    ...
    .execute(function(resourceGroup, vnetName, name, options, _) {
      ...
      if(options.privateAccessServices) {
        if(!utils.argHasValue(options.privateAccessServices)) {
          delete parameters.privateAccessServices;
        } else {
          parameters.privateAccessServices = options.privateAccessServices.split(',').map(function(item) { return { service: item }; });
        }
      }
