...
__.extend(VirtualNetworkGateway.prototype, {
  ...
  _parseConnection: function (resourceGroupName, connection, options, useDefaults, _) {
    ...
    if (options.vnetGateway1) {
      ...
      var vnetGatewayInfo = resourceUtils.getResourceInformation(connection.virtualNetworkGateway1.id);
      connection.virtualNetworkGateway1 = self.get(vnetGatewayInfo.resourceGroup, vnetGatewayInfo.resourceName, _);
    }
    ...
    if (options.vnetGateway2) {
      ...
      var vnetGatewayInfo = resourceUtils.getResourceInformation(connection.virtualNetworkGateway2.id);
      connection.virtualNetworkGateway2 = self.get(vnetGatewayInfo.resourceGroup, vnetGatewayInfo.resourceName, _);
    }
