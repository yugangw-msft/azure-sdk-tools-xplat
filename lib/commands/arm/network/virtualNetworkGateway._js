...
__.extend(VirtualNetworkGateway.prototype, {
  ...
  _parseConnection: function (resourceGroupName, connection, options, useDefaults, _) {
    ...
    /*@param {object} [parameters.peer] The reference to peerings resource.
     @param {string} [parameters.peer.id] Resource Id - of circuit
     */
    ...
    if (options.peerName) {
      var resGroupGateway2 = options.peerGroup || resourceGroupName;
      if (!options.peerGroup) {
        self.output.warn(util.format($('Using resource group for connected peer: %s'), resGroupGateway2));
      }

      var peer = self.expressRouteCrud.getCircuit(resGroupGateway2, options.peerName, _);
      if (!peer) throw new Error(util.format($('An express route circuit with name "%s" not found in the resource group "%s"'), options.peerName, resGroupGateway2));
