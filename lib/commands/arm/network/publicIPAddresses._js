    ...
    .execute(function(resourceGroup, options, _) {
      ...
      try {
        if(typeof networkManagementClient.publicIPAddresses.listAll != 'function') {
          ...
          publicIPAddress = networkManagementClient.publicIPAddresses.list(resourceGroup,  _);
        } else {
          if(options.resourceGroup) {
            ...
            progress = cli.interaction.progress($('Getting the public ip addresses'));
            publicIPAddress = networkManagementClient.publicIPAddresses.listAll(_);
          }
        }
      } finally {
        progress.end();
      ...
      cli.interaction.formatOutput(publicIPAddress, function(publicIPAddress) {
        if (publicIPAddress.length === 0) {
          cli.output.warn($('No public ip addresses found'));
        } else {
         cli.output.table(publicIPAddress, function (row, item) {
