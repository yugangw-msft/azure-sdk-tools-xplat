...
__.extend(VMClient.prototype, {
  ...
  setAzureDiskEncryption: function (resourceGroupName, name, options, _) {
    ...
    // Figure out if restart will be required
    var restartRequired = false;
    if (vmModel.storageProfile.osDisk && vmModel.storageProfile.osDisk.encryptionSettings &&
      vmModel.storageProfile.osDisk.encryptionSettings.enabled)
    {
    ...
    {
      virtualMachine.deallocateVM(resourceGroupName, name, _);
      vmModel = virtualMachine.getVM(resourceGroupName, name, _);
    }
    // update VM encryption settings
    vmModel.storageProfile.osDisk.encryptionSettings = encryptionSettings;
    var updateResult = virtualMachine.createOrUpdateVM(resourceGroupName, vmModel, false, _);
