...
var hdInsightCommandLine = function(cli, userInteractor, executionProcessor) {
  ...
  this.createClusterCommand = function(clusterName, options, _) {
    ...
    options.clusterType = self.user.promptIfNotGiven($('Cluster Type: '), option.clusterType, _);
    ...
    if (!utils.stringIsNullOrEmpty(options.directoryType) || 
        !utils.stringIsNullOrEmpty(options.domain) ||
        ...
        !utils.stringIsNullOrEmpty(options.clusterUsersGroupDNs)) {
          options.directoryType = self.user.promptIfNotGiven($('Security Profile - directory type: '), options.directoryType, _);
          ...
          options.ldapsUrls = self.user.promptIfNotGiven($('Security Profile - ldaps url: '), options.ldapsUrls, _);
