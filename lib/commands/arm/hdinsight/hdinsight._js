...
var hdInsightCommandLine = function(cli, userInteractor, executionProcessor) {
  ...
  this.createClusterCommand = function(clusterName, options, _) {
    ...
    var defaultStorageType = options.defaultStorageAccountName.indexOf('blob.core.windows.net') !== -1 ? 'WASB' : 'ADLS';
    if (utils.ignoreCaseEquals(defaultStorageType, 'WASB')){
    ...
    if (utils.ignoreCaseEquals(defaultStorageType, 'ADLS') || 
    !utils.stringIsNullOrEmpty(options.objectId) || 
    !utils.stringIsNullOrEmpty(options.aadTenantId) ||
    !utils.stringIsNullOrEmpty(options.resourceUri) || 
    !utils.stringIsNullOrEmpty(options.certificatePath) ||
    !utils.stringIsNullOrEmpty(options.certificatePassword)) {
      if(utils.stringIsNullOrEmpty(options.resourceUri)){
        options.resourceUri = 'https://management.core.windows.net/';
      }
      options.objectId = self.user.promptIfNotGiven($('Service Principal Object Id: '), options.objectId, _);
      options.aadTenantId = self.user.promptIfNotGiven($('Service Principal AAD Tenant Id: '), options.aadTenantId, _);
      options.certificatePath = self.user.promptIfNotGiven($('Service Principal Certificate file Path: '), options.certificatePath, _);
      options.certificatePassword = self.user.promptIfNotGiven($('Service Principal Certificate password: '), options.certificatePassword, _);
...
hdInsightCommandLine.init = function(cli) {
  ...
  cluster.command('create [clusterName]')
    ...
    .option('--defaultStorageAccountKey <storageAccountKey>', $('Key to the storage account to use for default HDInsight storage (only for clusters with WASB default Storage)'))
    .option('--defaultStorageContainer <storageContainer>', $('Container in the storage account to use for HDInsight default storage (only for clusters with WASB default Storage)'))
    .option('--defaultStorageRootPath <defaultStorageRootPath>', $('Path-prefix in the Azure Data Lake store account that this cluster will use as default File System ' + 
      '(only for clusters with Azure Data Lake Store as default Storage)'))
    .option('--objectId <objectId>', $('Service principal object Id used to access Azure Data Lake Store account (only for Data Lake enabled clusters)'))
    .option('--aadTenantId <aadTenantId>', $('Service principal AADTenant Id used to access Azure Data Lake Store account ' + 
      '(only for Data Lake enabled clusters)'))    
    .option('--certificatePath <certificatePath>', $('Service principal certificate file path used to access Azure Data Lake Store account ' +
      '(only for Data Lake enabled clusters)'))
    .option('--certificatePassword <certificatePassword>', $('Service principal certificate password used to access Azure Data Lake Store account ' + 
      '(only for Data Lake enabled clusters)')) 
    .option('--resourceUri <resourceUri>', $('Resource URI to access Azure Data Lake Store Account (only for Data Lake enabled clusters)'))
