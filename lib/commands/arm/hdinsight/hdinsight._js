...
var hdInsightCommandLine = function(cli, userInteractor, executionProcessor) {
  ...
  this.createClusterCommand = function(clusterName, options, _) {
    ...
    */
    ...
    !utils.stringIsNullOrEmpty(options.servicePrincipalAuthorityUri) || 
    ...
    !utils.stringIsNullOrEmpty(options.servicePrincipalCertPassword)) {
      ...
      options.servicePrincipalAuthorityUri = self.user.promptIfNotGiven($('Authority Uri to acquire access token for Service Principal: '), options.servicePrincipalAuthorityUri, _);
      ...
      options.clusterIdentity = {
        ...
        aadTenant: options.servicePrincipalAuthorityUri,
...
hdInsightCommandLine.init = function(cli) {
  ...
  cluster.command('create [clusterName]')
    ...
    .option('--servicePrincipalAuthorityUri <servicePrincipalAuthorityUri>', $('URI specifying the authority to acquire access token for Azure Data Lake Store ' + 
      'For example, "https://login.windows.net/<tenant>" (required only for Data Lake Store enabled clusters)'))    
