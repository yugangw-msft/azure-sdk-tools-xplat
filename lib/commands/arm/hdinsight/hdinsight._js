...
var hdInsightCommandLine = function(cli, userInteractor, executionProcessor) {
  ...
  this.createClusterCommand = function(clusterName, options, _) {
    ...
    */
    ...
    !utils.stringIsNullOrEmpty(options.datalakeResourceUri) || 
    !utils.stringIsNullOrEmpty(options.certificateFilePath) ||
    !utils.stringIsNullOrEmpty(options.certificatePassword)) {
      if(utils.stringIsNullOrEmpty(options.datalakeResourceUri)) {
        options.datalakeResourceUri = 'https://management.core.windows.net/';
      }
      ...
      options.certificateFilePath = self.user.promptIfNotGiven($('Service Principal Certificate file Path: '), options.certificateFilePath, _);
      ...
      options.clusterIdentity = {
        ...
        certificatePath: options.certificateFilePath,
...
hdInsightCommandLine.init = function(cli) {
  ...
  cluster.command('create [clusterName]')
    ...
    .option('--defaultStorageAccountKey <storageAccountKey>', $('Key to the Azure storage account to use for default HDInsight storage (only for clusters with WASB default storage)'))
    .option('--defaultStorageContainer <storageContainer>', $('Container in the Azure storage account to use for HDInsight default storage (only for clusters with WASB default storage)'))
    ...
    .option('--objectId <objectId>', $('Object Id of the service principal used to access Azure Data Lake Store account (only for Data Lake enabled clusters)'))
    ...
    .option('--certificateFilePath <certificateFilePath>', $('Service principal certificate file path used to access Azure Data Lake Store account ' +
      '(only for Data Lake enabled clusters)'))
    .option('--certificatePassword <certificatePassword>', $('Service principal certificate password used to access Azure Data Lake Store account ' + 
      '(only for Data Lake enabled clusters)')) 
    .option('--datalakeResourceUri <datalakeResourceUri>', $('Resource URI to access Azure Data Lake Store Account (only for Data Lake enabled clusters)'))
