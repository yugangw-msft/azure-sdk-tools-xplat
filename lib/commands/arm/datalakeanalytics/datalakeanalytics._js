...
exports.init = function (cli) {
  ...
  dataLakeAnalyticsAccountDataSource.command('add [accountName] [resource-group]')
    ...
    if (options.dataLakeStore) {
      client.dataLakeStoreAccounts.add(resourceGroup, accountName, options.dataLakeStore, _);
    }
    else {
      var parameters = {
      ...
      client.storageAccounts.add(resourceGroup, accountName, options.azureBlob, parameters, _);
    }
    
    log.info($('Successfully added the storage account specified to the Data Lake Analytics account: ' + accountName));
  });
  
  dataLakeAnalyticsAccountDataSource.command('delete [accountName] [resource-group]')
    ...
    if (options.dataLakeStore) {
      client.dataLakeStoreAccounts.deleteMethod(resourceGroup, accountName, options.dataLakeStore, _);
    }
    else {
      client.storageAccounts.deleteMethod(resourceGroup, accountName, options.azureBlob, _);
    }
