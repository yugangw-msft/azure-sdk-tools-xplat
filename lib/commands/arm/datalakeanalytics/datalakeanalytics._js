...
exports.init = function (cli) {
  ...
  dataLakeAnalyticsJob.command('wait [accountName] [jobId] [waitInterval] [timeout]')
    ...
    .option('-s, --subscription <id>', $('the subscription identifier'))
    .execute(function (accountName, jobId, waitInterval, timeout, options, _) {
    var subscription = profile.current.getSubscription(options.subscription);
    var client = utils.createDataLakeAnalyticsJobManagementClient(subscription);
    ...
    var jobToCheck = client.job.get(accountName, jobId, _);
    var timeWaited = 0;
    while (jobToCheck.state != 'Ended') {
      if (timeout > 0 && timeWaited >= timeout) {
        throw new Error($('Data Lake Analytics Job with ID: ' + jobId + ' has not completed in ' + timeout + ' seconds. Check job runtime or increase the value of --timeout'));
      }
      setTimeout(function () {
        log.info('Job is not yet done. Current job state: \'' + jobToCheck.state + '\'')
        timeWaited += waitInterval;
        jobToCheck = client.job.get(accountName, jobId, _);
      }, waitInterval * 1000);
    }
  ...
  dataLakeAnalyticsCatalogCredential.command('create [accountName] [databaseName] [hostUri] [credentialName] [credentialUserName]')
    ...
    .usage('[options] <accountName> <databaseName> <hostUri> <credentialName>')
    .option('-n --accountName <accountName>', $('The Data Lake Analytics account name to perform the action on.'))
  ...
  dataLakeAnalyticsCatalogCredential.command('delete [accountName] [databaseName] [credentialName]')
    ...
    client.catalog.deleteCredential(accountName, databaseName, credentialName, 
  });
  ...
  dataLakeAnalyticsAccountDataSource.command('set [accountName] [azureBlob] [accessKey] [resource-group]')
    ...
    client.storageAccounts.update(resourceGroup, accountName, azureBlob, options, _);
