...
exports.init = function (cli) {
  ...
  frontendIPConfigurations.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(loadBalancer.frontendIPConfigurations, { name: name })) {
        throw new Error(util.format($('frontend ip configuration with name "%s" already exists in the "%s"'), name, lbName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating frontend ip configuration in "%s"'), lbName));
      try {
  ...
  frontendIPConfigurations.command('set [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(!frontendIPConfiguration) {
        throw new Error(util.format($('frontend ip configuration with name "%s" not found in the "%s"'), name, lbName));
      }
  ...
  frontendIPConfigurations.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(loadBalancer.frontendIPConfigurations, {name: name});
      if (index === -1) {
        throw new Error(util.format($('frontend ip configuration "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete frontend ip configuration with name "%s" from "%s"? [y/n] '), name, lbName), _)) {
        return;
      }
  ...
  frontendIPConfigurations.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var frontendIPConfiguration = utils.findFirstCaseIgnore(loadBalancer.frontendIPConfigurations, { name: name });
      if(!frontendIPConfiguration) {
        cli.output.warn(util.format($('frontend ip configuration with name "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }
