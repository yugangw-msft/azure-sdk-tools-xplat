...
exports.init = function (cli) {
  ...
  httpListeners.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.httpListeners, { name: name })) {
        throw new Error(util.format($('http listener with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating http listener in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  httpListeners.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!httpListener) {
        throw new Error(util.format($('http listener with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  httpListeners.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.httpListeners, {name: name});
      if (index === -1) {
        throw new Error(util.format($('http listener "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete http listener with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  httpListeners.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var httpListener = utils.findFirstCaseIgnore(applicationGateway.httpListeners, { name: name });
      if(!httpListener) {
        cli.output.warn(util.format($('http listener with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
