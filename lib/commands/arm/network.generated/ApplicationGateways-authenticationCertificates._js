...
exports.init = function (cli) {
  ...
  authenticationCertificates.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.authenticationCertificates, { name: name })) {
        throw new Error(util.format($('authentication certificate with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating authentication certificate in "%s"'), gatewayName));
      try {
  ...
  authenticationCertificates.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!authenticationCertificate) {
        throw new Error(util.format($('authentication certificate with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  authenticationCertificates.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.authenticationCertificates, {name: name});
      if (index === -1) {
        throw new Error(util.format($('authentication certificate "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete authentication certificate with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  authenticationCertificates.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var authenticationCertificate = utils.findFirstCaseIgnore(applicationGateway.authenticationCertificates, { name: name });
      if(!authenticationCertificate) {
        cli.output.warn(util.format($('authentication certificate with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
