...
exports.init = function (cli) {
  ...
  probes.command('create [resource-group] [gateway-name] [name] [path] [timeout]')
    ...
    .execute(function(resourceGroup, gatewayName, name, path, timeout, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.probes, { name: name })) {
        throw new Error(util.format($('probe with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating probe in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  probes.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!probe) {
        throw new Error(util.format($('probe with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  probes.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.probes, {name: name});
      if (index === -1) {
        throw new Error(util.format($('probe "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete probe with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  probes.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var probe = utils.findFirstCaseIgnore(applicationGateway.probes, { name: name });
      if(!probe) {
        cli.output.warn(util.format($('probe with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
