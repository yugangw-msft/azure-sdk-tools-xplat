...
exports.init = function (cli) {
  ...
  probes.command('create [resource-group] [gateway-name] [name] [path] [timeout]')
    ...
    .option('-r, --health-response-body [health-response-body]', $('health response body option value'))
    .option('-c, --status-codes [status-codes]', $('status codes option value'))
    ...
    .execute(function(resourceGroup, gatewayName, name, path, timeout, options, _) {
      ...
      if (probe) {
        throw new Error(util.format($('probe with name "%s" already exists in application gateway "%s"'), name, gatewayName));
      }
      ...
      if(options.hostName) {
        parameters.host = options.hostName;
      }
      ...
      if(options.interval) {
        parameters.interval = parseInt(options.interval, 10);
      }

      if(options.timeout) {
        parameters.timeout = parseInt(options.timeout, 10);
      }

      if(options.unhealthyThreshold) {
        parameters.unhealthyThreshold = parseInt(options.unhealthyThreshold, 10);
      }

      if(options.pickHostName) {
        parameters.pickHostNameFromBackendHttpSettings = utils.parseBool(options.pickHostName);
      }

      if(options.minServers) {
        parameters.minServers = parseInt(options.minServers, 10);
      }
      ...
      if(options.healthResponseBody) {
        parameters.match.body = options.healthResponseBody;
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating probe in application gateway "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  probes.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var probe = utils.findFirstCaseIgnore(parentItem.probes, { name: name });
      if (!probe) {
        throw new Error(util.format($('probe with name "%s" not found in application gateway "%s"'), name, gatewayName));
      }
      ...
      generatorUtils.removeEmptyObjects(parameters);
      progress = cli.interaction.progress(util.format($('Updating probe in "%s"'), gatewayName));
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        } else {
  ...
  probes.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if (index === -1) {
        throw new Error(util.format($('probe "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete probe with name "%s" from application gateway "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  probes.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var probe = utils.findFirstCaseIgnore(parentItem.probes, { name: name });
      if (!probe) {
        cli.output.warn(util.format($('probe with name "%s" not found in the application gateway "%s"'), name, gatewayName));
      }
