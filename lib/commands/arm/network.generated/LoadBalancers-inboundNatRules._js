...
exports.init = function (cli) {
  ...
  inboundNatRules.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(loadBalancer.inboundNatRules, { name: name })) {
        throw new Error(util.format($('inbound nat rule with name "%s" already exists in the "%s"'), name, lbName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating inbound nat rule in "%s"'), lbName));
      try {
  ...
  inboundNatRules.command('set [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(!inboundNatRule) {
        throw new Error(util.format($('inbound nat rule with name "%s" not found in the "%s"'), name, lbName));
      }
  ...
  inboundNatRules.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(loadBalancer.inboundNatRules, {name: name});
      if (index === -1) {
        throw new Error(util.format($('inbound nat rule "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete inbound nat rule with name "%s" from "%s"? [y/n] '), name, lbName), _)) {
        return;
      }
  ...
  inboundNatRules.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var inboundNatRule = utils.findFirstCaseIgnore(loadBalancer.inboundNatRules, { name: name });
      if(!inboundNatRule) {
        cli.output.warn(util.format($('inbound nat rule with name "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }
