...
exports.init = function (cli) {
  ...
  sslPolicy.command('create [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var sslPolicy = parentItem.sslPolicy;
      if (sslPolicy) {
        throw new Error(util.format($('ssl policy already exists in application gateway "%s"'), gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating ssl policy in application gateway "%s"'), gatewayName));
      try {
      ...
      cli.interaction.formatOutput(parentItem.sslPolicy, generatorUtils.traverse);
    });
  ...
  sslPolicy.command('set [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var sslPolicy = parentItem.sslPolicy;
      if (!sslPolicy) {
        throw new Error(util.format($('ssl policy not found in application gateway "%s"'), gatewayName));
      }

      var parameters = sslPolicy;
  ...
  sslPolicy.command('delete [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete ssl policy from application gateway "%s"? [y/n] '), gatewayName), _)) {
        return;
      }
