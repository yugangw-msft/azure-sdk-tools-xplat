...
exports.init = function (cli) {
  ...
  sslCertificates.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.sslCertificates, { name: name })) {
        throw new Error(util.format($('ssl certificate with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating ssl certificate in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  sslCertificates.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!sslCertificate) {
        throw new Error(util.format($('ssl certificate with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  sslCertificates.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.sslCertificates, {name: name});
      if (index === -1) {
        throw new Error(util.format($('ssl certificate "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete ssl certificate with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  sslCertificates.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var sslCertificate = utils.findFirstCaseIgnore(applicationGateway.sslCertificates, { name: name });
      if(!sslCertificate) {
        cli.output.warn(util.format($('ssl certificate with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
