...
exports.init = function (cli) {
  ...
  sslCertificates.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var sslCertificate = utils.findFirstCaseIgnore(parentItem.sslCertificates, { name: name });
      if (sslCertificate) {
        throw new Error(util.format($('ssl certificate with name "%s" already exists in application gateway "%s"'), name, gatewayName));
      }

      var parameters = {};

      if(options.certPassword) {
        parameters.password = options.certPassword;
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating ssl certificate in application gateway "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  sslCertificates.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var sslCertificate = utils.findFirstCaseIgnore(parentItem.sslCertificates, { name: name });
      if (!sslCertificate) {
        throw new Error(util.format($('ssl certificate with name "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      var parameters = sslCertificate;
      ...
      generatorUtils.removeEmptyObjects(parameters);
      progress = cli.interaction.progress(util.format($('Updating ssl certificate in "%s"'), gatewayName));
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        } else {
  ...
  sslCertificates.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(parentItem.sslCertificates, { name: name });
      if (index === -1) {
        throw new Error(util.format($('ssl certificate "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete ssl certificate with name "%s" from application gateway "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }

      parentItem.sslCertificates.splice(index, 1);

      progress = cli.interaction.progress('Deleting ssl certificate');
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        }
