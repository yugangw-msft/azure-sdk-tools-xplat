...
exports.init = function (cli) {
  ...
  backendAddressPools.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if (backendAddressPool) {
        throw new Error(util.format($('backend address pool with name "%s" already exists in load balancer "%s"'), name, lbName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating backend address pool in load balancer "%s"'), lbName));
      try {
      ...
      cli.interaction.formatOutput(utils.findFirstCaseIgnore(parentItem.backendAddressPools, { name: name }), generatorUtils.traverse);
    });
  ...
  backendAddressPools.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if (index === -1) {
        throw new Error(util.format($('backend address pool "%s" not found in load balancer "%s"'), name, lbName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete backend address pool with name "%s" from load balancer "%s"? [y/n] '), name, lbName), _)) {
        return;
      }
  ...
  backendAddressPools.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var backendAddressPool = utils.findFirstCaseIgnore(parentItem.backendAddressPools, { name: name });
      if (!backendAddressPool) {
        cli.output.warn(util.format($('backend address pool with name "%s" not found in the load balancer "%s"'), name, lbName));
      }
