...
exports.init = function (cli) {
  ...
  backendAddressPools.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(loadBalancer.backendAddressPools, { name: name })) {
        throw new Error(util.format($('backend address pool with name "%s" already exists in the "%s"'), name, lbName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating backend address pool in "%s"'), lbName));
      try {
  ...
  backendAddressPools.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if (index === -1) {
        throw new Error(util.format($('backend address pool "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete backend address pool with name "%s" from "%s"? [y/n] '), name, lbName), _)) {
        return;
      }
  ...
  backendAddressPools.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(!backendAddressPool) {
        cli.output.warn(util.format($('backend address pool with name "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }
