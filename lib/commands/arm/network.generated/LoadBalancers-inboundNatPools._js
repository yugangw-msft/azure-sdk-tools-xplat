...
exports.init = function (cli) {
  ...
  inboundNatPools.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(loadBalancer.inboundNatPools, { name: name })) {
        throw new Error(util.format($('inbound nat pool with name "%s" already exists in the "%s"'), name, lbName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating inbound nat pool in "%s"'), lbName));
      try {
  ...
  inboundNatPools.command('set [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(!inboundNatPool) {
        throw new Error(util.format($('inbound nat pool with name "%s" not found in the "%s"'), name, lbName));
      }
  ...
  inboundNatPools.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(loadBalancer.inboundNatPools, {name: name});
      if (index === -1) {
        throw new Error(util.format($('inbound nat pool "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete inbound nat pool with name "%s" from "%s"? [y/n] '), name, lbName), _)) {
        return;
      }
  ...
  inboundNatPools.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var inboundNatPool = utils.findFirstCaseIgnore(loadBalancer.inboundNatPools, { name: name });
      if(!inboundNatPool) {
        cli.output.warn(util.format($('inbound nat pool with name "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }
