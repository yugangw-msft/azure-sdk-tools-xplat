...
exports.init = function (cli) {
  ...
  backendAddressPools.command('create [resource-group] [gateway-name] [name] [servers]')
    ...
    .execute(function(resourceGroup, gatewayName, name, servers, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.backendAddressPools, { name: name })) {
        throw new Error(util.format($('backend address pool with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating backend address pool in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  backendAddressPools.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!backendAddressPool) {
        throw new Error(util.format($('backend address pool with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  backendAddressPools.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.backendAddressPools, {name: name});
      if (index === -1) {
        throw new Error(util.format($('backend address pool "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete backend address pool with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  backendAddressPools.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var backendAddressPool = utils.findFirstCaseIgnore(applicationGateway.backendAddressPools, { name: name });
      if(!backendAddressPool) {
        cli.output.warn(util.format($('backend address pool with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
