...
exports.init = function (cli) {
  ...
  backendAddressPools.command('create [resource-group] [gateway-name] [name] [servers]')
    ...
    .execute(function(resourceGroup, gatewayName, name, servers, options, _) {
      ...
      var backendAddressPool = utils.findFirstCaseIgnore(parentItem.backendAddressPools, { name: name });
      if (backendAddressPool) {
        throw new Error(util.format($('backend address pool with name "%s" already exists in application gateway "%s"'), name, gatewayName));
      }

      var parameters = {};

      if(options.servers) {
        parameters.backendAddresses = options.servers.split(',').map(function (item) { return { ipAddress: item }; });
      }

      parameters.name = name;
      parentItem.backendAddressPools.push(parameters);

      generatorUtils.removeEmptyObjects(parameters);
      progress = cli.interaction.progress(util.format($('Creating backend address pool in application gateway "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  backendAddressPools.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var backendAddressPool = utils.findFirstCaseIgnore(parentItem.backendAddressPools, { name: name });
      if (!backendAddressPool) {
        throw new Error(util.format($('backend address pool with name "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      var parameters = backendAddressPool;
      ...
      generatorUtils.removeEmptyObjects(parameters);
      progress = cli.interaction.progress(util.format($('Updating backend address pool in "%s"'), gatewayName));
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        } else {
  ...
  backendAddressPools.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(parentItem.backendAddressPools, { name: name });
      if (index === -1) {
        throw new Error(util.format($('backend address pool "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete backend address pool with name "%s" from application gateway "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }

      parentItem.backendAddressPools.splice(index, 1);

      progress = cli.interaction.progress('Deleting backend address pool');
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        }
  ...
  backendAddressPools.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var backendAddressPool = utils.findFirstCaseIgnore(parentItem.backendAddressPools, { name: name });
      if (!backendAddressPool) {
        cli.output.warn(util.format($('backend address pool with name "%s" not found in the application gateway "%s"'), name, gatewayName));
      }
  ...
  backendAddressPools.command('list [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      cli.interaction.formatOutput(parentItem.backendAddressPools, function (items) {
