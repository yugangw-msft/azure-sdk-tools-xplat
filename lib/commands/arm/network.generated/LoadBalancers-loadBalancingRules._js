...
exports.init = function (cli) {
  ...
  loadBalancingRules.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(loadBalancer.loadBalancingRules, { name: name })) {
        throw new Error(util.format($('load balancing rule with name "%s" already exists in the "%s"'), name, lbName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating load balancing rule in "%s"'), lbName));
      try {
  ...
  loadBalancingRules.command('set [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(!loadBalancingRule) {
        throw new Error(util.format($('load balancing rule with name "%s" not found in the "%s"'), name, lbName));
      }
  ...
  loadBalancingRules.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(loadBalancer.loadBalancingRules, {name: name});
      if (index === -1) {
        throw new Error(util.format($('load balancing rule "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete load balancing rule with name "%s" from "%s"? [y/n] '), name, lbName), _)) {
        return;
      }
  ...
  loadBalancingRules.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var loadBalancingRule = utils.findFirstCaseIgnore(loadBalancer.loadBalancingRules, { name: name });
      if(!loadBalancingRule) {
        cli.output.warn(util.format($('load balancing rule with name "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }
