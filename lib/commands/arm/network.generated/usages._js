...
exports.init = function (cli) {
  ...
  var usages = network.category('usage')
    .description($('Commands to manage usages'));

  usages.command('list [location]')
    ...
    .execute(function(location, options, _) {
      ...
      try {
        if(typeof networkManagementClient.usages.listAll != 'function') {
          ...
          usage = networkManagementClient.usages.list(location,  _);
        } else {
          if(options.resourceGroup) {
            ...
            progress = cli.interaction.progress($('Getting the usages'));
            usage = networkManagementClient.usages.listAll(_);
          }
        }
      } finally {
        progress.end();
