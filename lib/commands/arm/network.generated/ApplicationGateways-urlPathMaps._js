...
exports.init = function (cli) {
  ...
  urlPathMaps.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.urlPathMaps, { name: name })) {
        throw new Error(util.format($('url path map with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating url path map in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  urlPathMaps.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if (index === -1) {
        throw new Error(util.format($('url path map "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete url path map with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  urlPathMaps.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!urlPathMap) {
        cli.output.warn(util.format($('url path map with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
