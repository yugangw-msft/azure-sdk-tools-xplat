...
exports.init = function (cli) {
  ...
  backendHttpSettingsCollection.command('create [resource-group] [gateway-name] [name] [port]')
    ...
    .execute(function(resourceGroup, gatewayName, name, port, options, _) {
      ...
      var backendHttpSettingsCollection = utils.findFirstCaseIgnore(parentItem.backendHttpSettingsCollection, { name: name });
      if (backendHttpSettingsCollection) {
        throw new Error(util.format($('backend http settings collection with name "%s" already exists in application gateway "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating backend http settings collection in application gateway "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  backendHttpSettingsCollection.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var backendHttpSettingsCollection = utils.findFirstCaseIgnore(parentItem.backendHttpSettingsCollection, { name: name });
      if (!backendHttpSettingsCollection) {
        throw new Error(util.format($('backend http settings collection with name "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      var parameters = backendHttpSettingsCollection;
  ...
  backendHttpSettingsCollection.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(parentItem.backendHttpSettingsCollection, { name: name });
      if (index === -1) {
        throw new Error(util.format($('backend http settings collection "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete backend http settings collection with name "%s" from application gateway "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
