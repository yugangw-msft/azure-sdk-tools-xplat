...
exports.init = function (cli) {
  ...
  webApplicationFirewallConfiguration.command('create [resource-group] [gateway-name] [enable] [waf-mode] [rule-set-type] [rule-set-version]')
    ...
    .execute(function(resourceGroup, gatewayName, enable, wafMode, ruleSetType, ruleSetVersion, options, _) {
      ...
      var webApplicationFirewallConfiguration = parentItem.webApplicationFirewallConfiguration;
      if (webApplicationFirewallConfiguration) {
        throw new Error(util.format($('web application firewall configuration already exists in application gateway "%s"'), gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating web application firewall configuration in application gateway "%s"'), gatewayName));
      try {
        if (options.nowait) {
      ...
      cli.interaction.formatOutput(parentItem.webApplicationFirewallConfiguration, generatorUtils.traverse);
    });
  ...
  webApplicationFirewallConfiguration.command('set [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      var webApplicationFirewallConfiguration = parentItem.webApplicationFirewallConfiguration;
      if (!webApplicationFirewallConfiguration) {
        throw new Error(util.format($('web application firewall configuration not found in application gateway "%s"'), gatewayName));
      }

      var parameters = webApplicationFirewallConfiguration;
  ...
  webApplicationFirewallConfiguration.command('delete [resource-group] [gateway-name]')
    ...
    .execute(function(resourceGroup, gatewayName, options, _) {
      ...
      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete web application firewall configuration from application gateway "%s"? [y/n] '), gatewayName), _)) {
        return;
      }
