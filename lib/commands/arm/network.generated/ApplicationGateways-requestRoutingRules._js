...
exports.init = function (cli) {
  ...
  requestRoutingRules.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.requestRoutingRules, { name: name })) {
        throw new Error(util.format($('request routing rule with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating request routing rule in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  requestRoutingRules.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!requestRoutingRule) {
        throw new Error(util.format($('request routing rule with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  requestRoutingRules.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.requestRoutingRules, {name: name});
      if (index === -1) {
        throw new Error(util.format($('request routing rule "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete request routing rule with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  requestRoutingRules.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var requestRoutingRule = utils.findFirstCaseIgnore(applicationGateway.requestRoutingRules, { name: name });
      if(!requestRoutingRule) {
        cli.output.warn(util.format($('request routing rule with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
