...
exports.init = function (cli) {
  ...
  requestRoutingRules.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var requestRoutingRule = utils.findFirstCaseIgnore(parentItem.requestRoutingRules, { name: name });
      if (requestRoutingRule) {
        throw new Error(util.format($('request routing rule with name "%s" already exists in application gateway "%s"'), name, gatewayName));
      }
      ...
      if(options.addressPoolId) {
        ...
        parameters.backendAddressPool.id = options.addressPoolId;
      ...
      if(options.httpSettingsId) {
        ...
        parameters.backendHttpSettings.id = options.httpSettingsId;
      ...
      if(options.httpListenerId) {
        ...
        parameters.httpListener.id = options.httpListenerId;
      ...
      if(options.urlPathMapId) {
        ...
        parameters.urlPathMap.id = options.urlPathMapId;
      ...
      if(options.redirectConfigurationId) {
        ...
        parameters.redirectConfiguration.id = options.redirectConfigurationId;
      ...
      progress = cli.interaction.progress(util.format($('Creating request routing rule in application gateway "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  requestRoutingRules.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var requestRoutingRule = utils.findFirstCaseIgnore(parentItem.requestRoutingRules, { name: name });
      if (!requestRoutingRule) {
        throw new Error(util.format($('request routing rule with name "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      var parameters = requestRoutingRule;
      ...
      generatorUtils.removeEmptyObjects(parameters);
      progress = cli.interaction.progress(util.format($('Updating request routing rule in "%s"'), gatewayName));
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        } else {
  ...
  requestRoutingRules.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(parentItem.requestRoutingRules, { name: name });
      if (index === -1) {
        throw new Error(util.format($('request routing rule "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete request routing rule with name "%s" from application gateway "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }

      parentItem.requestRoutingRules.splice(index, 1);

      progress = cli.interaction.progress('Deleting request routing rule');
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        }
