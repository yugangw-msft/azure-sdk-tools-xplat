...
exports.init = function (cli) {
  ...
  frontendPorts.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.frontendPorts, { name: name })) {
        throw new Error(util.format($('frontend port with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating frontend port in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  frontendPorts.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!frontendPort) {
        throw new Error(util.format($('frontend port with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  frontendPorts.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.frontendPorts, {name: name});
      if (index === -1) {
        throw new Error(util.format($('frontend port "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete frontend port with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  frontendPorts.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var frontendPort = utils.findFirstCaseIgnore(applicationGateway.frontendPorts, { name: name });
      if(!frontendPort) {
        cli.output.warn(util.format($('frontend port with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
