...
exports.init = function (cli) {
  ...
  redirectConfigurations.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var redirectConfiguration = utils.findFirstCaseIgnore(parentItem.redirectConfigurations, { name: name });
      if (redirectConfiguration) {
        throw new Error(util.format($('redirect configuration with name "%s" already exists in application gateway "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating redirect configuration in application gateway "%s"'), gatewayName));
      try {
  ...
  redirectConfigurations.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var redirectConfiguration = utils.findFirstCaseIgnore(parentItem.redirectConfigurations, { name: name });
      if (!redirectConfiguration) {
        throw new Error(util.format($('redirect configuration with name "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      var parameters = redirectConfiguration;
      ...
      generatorUtils.removeEmptyObjects(parameters);
      progress = cli.interaction.progress(util.format($('Updating redirect configuration in "%s"'), gatewayName));
      try {
        applicationGateway = networkManagementClient.applicationGateways.createOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
      } finally {
        progress.end();
  ...
  redirectConfigurations.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(parentItem.redirectConfigurations, { name: name });
      if (index === -1) {
        throw new Error(util.format($('redirect configuration "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete redirect configuration with name "%s" from application gateway "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }

      parentItem.redirectConfigurations.splice(index, 1);

      progress = cli.interaction.progress('Deleting redirect configuration');
      try {
        applicationGateway = networkManagementClient.applicationGateways.createOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
      } finally {
        progress.end();
