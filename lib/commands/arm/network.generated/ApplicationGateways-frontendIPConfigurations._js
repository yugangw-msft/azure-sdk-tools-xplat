...
exports.init = function (cli) {
  ...
  frontendIPConfigurations.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(applicationGateway.frontendIPConfigurations, { name: name })) {
        throw new Error(util.format($('frontend ip configuration with name "%s" already exists in the "%s"'), name, gatewayName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating frontend ip configuration in "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  frontendIPConfigurations.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if(!frontendIPConfiguration) {
        throw new Error(util.format($('frontend ip configuration with name "%s" not found in the "%s"'), name, gatewayName));
      }
  ...
  frontendIPConfigurations.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(applicationGateway.frontendIPConfigurations, {name: name});
      if (index === -1) {
        throw new Error(util.format($('frontend ip configuration "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete frontend ip configuration with name "%s" from "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  frontendIPConfigurations.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var frontendIPConfiguration = utils.findFirstCaseIgnore(applicationGateway.frontendIPConfigurations, { name: name });
      if(!frontendIPConfiguration) {
        cli.output.warn(util.format($('frontend ip configuration with name "%s" not found in the gatewayName "%s"'), name, applicationGateway.name));
      }
