...
exports.init = function (cli) {
  ...
  frontendIPConfigurations.command('create [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if (frontendIPConfiguration) {
        throw new Error(util.format($('frontend ip configuration with name "%s" already exists in application gateway "%s"'), name, gatewayName));
      }

      var parameters = {};

      if(options.staticIpAddress) {
        parameters.privateIPAddress = options.staticIpAddress;
      ...
      progress = cli.interaction.progress(util.format($('Creating frontend ip configuration in application gateway "%s"'), gatewayName));
      try {
        if (options.nowait) {
  ...
  frontendIPConfigurations.command('set [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var frontendIPConfiguration = utils.findFirstCaseIgnore(parentItem.frontendIPConfigurations, { name: name });
      if (!frontendIPConfiguration) {
        throw new Error(util.format($('frontend ip configuration with name "%s" not found in application gateway "%s"'), name, gatewayName));
      }
      ...
      generatorUtils.removeEmptyObjects(parameters);
      progress = cli.interaction.progress(util.format($('Updating frontend ip configuration in "%s"'), gatewayName));
      try {
        if (options.nowait) {
          applicationGateway = networkManagementClient.applicationGateways.beginCreateOrUpdate(resourceGroup, gatewayName, applicationGateway, _);
        } else {
  ...
  frontendIPConfigurations.command('delete [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      if (index === -1) {
        throw new Error(util.format($('frontend ip configuration "%s" not found in application gateway "%s"'), name, gatewayName));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete frontend ip configuration with name "%s" from application gateway "%s"? [y/n] '), name, gatewayName), _)) {
        return;
      }
  ...
  frontendIPConfigurations.command('show [resource-group] [gateway-name] [name]')
    ...
    .execute(function(resourceGroup, gatewayName, name, options, _) {
      ...
      var frontendIPConfiguration = utils.findFirstCaseIgnore(parentItem.frontendIPConfigurations, { name: name });
      if (!frontendIPConfiguration) {
        cli.output.warn(util.format($('frontend ip configuration with name "%s" not found in the application gateway "%s"'), name, gatewayName));
      }
