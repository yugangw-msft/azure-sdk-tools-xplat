...
exports.init = function (cli) {
  ...
  probes.command('create [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(utils.findFirstCaseIgnore(loadBalancer.probes, { name: name })) {
        throw new Error(util.format($('probe with name "%s" already exists in the "%s"'), name, lbName));
      }
      ...
      progress = cli.interaction.progress(util.format($('Creating probe in "%s"'), lbName));
      try {
  ...
  probes.command('set [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      if(!probe) {
        throw new Error(util.format($('probe with name "%s" not found in the "%s"'), name, lbName));
      }
  ...
  probes.command('delete [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var index = utils.indexOfCaseIgnore(loadBalancer.probes, {name: name});
      if (index === -1) {
        throw new Error(util.format($('probe "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }

      if (!options.quiet && !cli.interaction.confirm(util.format($('Delete probe with name "%s" from "%s"? [y/n] '), name, lbName), _)) {
        return;
      }
  ...
  probes.command('show [resource-group] [lb-name] [name]')
    ...
    .execute(function(resourceGroup, lbName, name, options, _) {
      ...
      var probe = utils.findFirstCaseIgnore(loadBalancer.probes, { name: name });
      if(!probe) {
        cli.output.warn(util.format($('probe with name "%s" not found in the lbName "%s"'), name, loadBalancer.name));
      }
