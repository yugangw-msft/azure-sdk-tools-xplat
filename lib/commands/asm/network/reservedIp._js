...
__.extend(ReservedIp.prototype, {
  create: function (name, location, options, _) {
    ...
    if (options.serviceName) {
      ...
      params.deploymentName = options.serviceName;
    }
  ...
  associate: function(name, serviceName, options, _) {
    var self = this;
    var params = {
      deploymentName: serviceName,
      serviceName: serviceName
    };
    var progress = self.interaction.progress($('Setting reserved IP address and cloud service association'));
    try {
      self.networkManagementClient.reservedIPs.associate(name, params, _);
    } finally {
      progress.end();
    }
  },

  disassociate: function(name, serviceName, options, _) {
    var self = this;
    ...
    var progress = self.interaction.progress($('Removing reserved IP address and cloud service association'));
    try {
      self.networkManagementClient.reservedIPs.disassociate(name, params, _);
    } finally {
      progress.end();
    }
  }
